...
echo '<h3 class="module-title"><span><span>'.JText::_('COM_VM2TAGS_TAGGED_AS').': '.$tag.'</h3></span></span>';
                            ...
                            <div class="slide-hover">
                                ...
                                <?php 
									...
									foreach($products as $prod){
										//var_dump($prod);
									if ($product->virtuemart_product_id == $prod->virtuemart_product_id){
 			
											 echo '<div class="product_tags" >';
				
								//$product_tags = '{"product_tags":"'.$product->tags.'"}';
							   // $accented_tags = $prod->accented_tags;
								//$accented_tags = explode(',',$accented_tags);
							
								$sep_tags = explode(',',$prod->accented_tags);
									echo '<i class="icon-tags" title="'.JText::_('COM_VM2TAGS_TAGS').'"  class="hasTooltip" ></i> ';
									for($i=0; $i< count($sep_tags);$i++){
										if($sep_tags[$i] !='' && $sep_tags[$i]!=' ')
											echo '<a class="vm_tags" href="'.JRoute::_('index.php?option=com_vm2tags&view=productslist&tag='.$sep_tags[$i].'&Itemid='.$vm2tags_itemid).'">'.$sep_tags[$i].'</a>&nbsp; ';
									}
