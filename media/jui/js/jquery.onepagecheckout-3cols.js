var geoip_country_name,geoip_city,requestUpdate = false;;
;(function(nx, $) {
	var $cache = {},
		isGuest,
		visited=false;
	...
	function toggle_pnlShipTo () {
		if(UI.cbSTsameAsBT.prop("checked")==true) {
			frmCheckOut.find('#shipping_info').fadeOut();
			copy_Fields();
			$('#shipto_virtuemart_country_id').trigger('change');
		} else {
			frmCheckOut.find('#shipping_info').fadeIn();
		}
			...
			$.ajax({
				...
				success : function (rs){
						...
						if(rs.error) {
							...
							$('input[type="radio"]','#shipments').click(function() {
								update_form();
							})
							 ...
							 * payments
							 */
							$('input[type="radio"]','#payments').click(function() {
								update_form();
							})
...
//		if(UI.cbRegister.length > 0) {
//			if(UI.cbRegister.prop('checked') == true) {
...
//						{
//							type: "POST",
//							data: $('#checkoutForm').serialize()+'&'+vmConfig.token+'=1'
//						}, 
//						function(res) {
...
//		}
		...
		if(!valid_billto) {
			...
			alert(Joomla.JText._('SYSTEM_ONESTEPCHECKOUT_FORM_INVALID'));
			return;
		}
	...
	function init_Cache() {
		...
		fBillToZip = flBillTo.filter('#billto_zip_field')
	...
	function init_Events() {
		...
		if(visited == null && UI.cbSTsameAsBT.prop("checked")== false) {UI.cbSTsameAsBT.prop("checked",true)}
		visited = 1;
		 $.cookies.get('visited',visited);
		/*
		 * login pane
		 */
