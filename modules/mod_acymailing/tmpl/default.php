...
<?php 
if($params->get('effect','normal') == 'popup-box'){
$document = JFactory::getDocument();
$document->addStyleSheet(JURI::root(true).'/modules/mod_acymailing/assets/jquery.fancybox.css');
	?>
 <script type="text/javascript" src="<?php echo JURI::base()?>modules/mod_acymailing/assets/Cookie.js"></script>
...
<a id="signup" href="#newsletter-popup"></a>
<?php }?>
<?php 
if($params->get('effect','normal') == 'popup-box'){ ?>
...
<?php } ?>
	<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		...
		<div class="acymailing_module_form" >
           ...
           <?php  if($params->get('effect','normal') == 'popup-box'){  ?>
           <tr>
		...
		if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
				 ...
				 if($params->get('showterms',false)){
					...
					<td <?php if($displayOutside AND !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
						...
						<input class="button unsubbutton reset" type="button" value="<?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?>" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"/>
						<?php } ?>
					</td>
				</tr>
		  <?php }else { ?>
