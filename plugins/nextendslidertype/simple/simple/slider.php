            ...
            <?php if ($flux[0]): ?>
                <div class="nextend-flux">
                    <?php foreach ($this->_slides AS $slide): ?>
                        <?php if ($slide['bg']): ?>
                            <img src="<?php echo ($slide['bg'] ? $slide['bg'] : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'); ?>" class="nextend-slide-bg"<?php if ($slide['first']) echo ' style="z-index:2;position: absolute; top: 0px; left: 0px;" '; ?>/>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
