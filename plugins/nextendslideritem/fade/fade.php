...
'<div style="'.htmlspecialchars($data->get('css', '')).';line-height:0;position: relative;display: block;" class="nextend-smartslider-fade-container nextend-smartslider-fade-'.$data->get('fadeclass', '').'" '.$attr.'>
  '.($link[0] != '#' ? '<a href="'.$link[0].'" target="'.$link[1].'" style="background: none !important; display:block;">' : '').'
    <span class="nextend-smartslider-fade">
        <img alt="'.htmlspecialchars($data->get('alt', '')).'" src="'.$data->get('imagefront', '').'" style="max-width: 100%; width: '.$data->get('width', '').';" class="nextend-smartslider-fade-front-img" >
        <img alt="'.htmlspecialchars($data->get('alt', '')).'" src="'.$data->get('imageback', '').'" style="max-width: 100%; width: '.$data->get('width', '').';" class="nextend-smartslider-fade-back-img" >
    </span>
  '.($link[0] != '#' ? '</a>' : '').'
  '.$css.'
</div>';
