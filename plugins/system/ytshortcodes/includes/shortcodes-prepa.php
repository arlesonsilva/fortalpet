...
function accItemShortcode($atts, $content = null){
	...
	$acc_item = $acc_item . "<i class='fa-plus-circle'></i>";
...
function galleryItemShortcode($atts, $content = null){
	...
	$gallery_item .= "<div class='image-overlay'><i class='fa fa-search-plus'></i><img src='" .$simage."' title='" . $title . "' alt='" . $title . "' /></div>";
	$gallery_item .= "</a><h4 class='item-gallery-title'>". $txt ."</h4><span>". $desc ."</span>";
	$gallery_item .= "</div>";
	$gallery_item .= "</li>";
...
function lightboxShortcode($atts){
	...
	if($width != "auto" && $width > 0 && $height != "auto" && $height > 0) {
		$isrc =ImageHelper::init($src, $small_image)->src();
	}else{
		$isrc = $src;
	}

	$frame = "<img src='".JURI::base().'/'.$isrc . "' alt='" . $title . "' />";
	...
	if($lightbox == 'On' || $lightbox == 'on') {
		$frame = "<a href='".JURI::base(true).'/'. $src . "' data-rel='prettyPhoto' title='" . $title . "' >" . $frame . $titles. $borderinner. "<i class='fa fa-search-plus'></i></a>";
	}
