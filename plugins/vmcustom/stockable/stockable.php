		...
		jQuery( function($) {
			...
			function recalculate_'.$js_suffix.'(stockableBlockIndex, Opt){
				...
				if ("disableadd" == stockhandle && found_child.in_stock <= 0) {
					$("#productdetailsview .addtocart-bar2>span").remove();
					$("#productdetailsview .addtocart-bar2>div").remove();
					$("#productdetailsview .addtocart-bar2>a.notify").remove();
					$("#productdetailsview .addtocart-bar2").append(\'<a href="ind\'+\'ex.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=\' + found_id + \'" class="notify addtocart-button hasTooltip">' . JText::_('COM_VIRTUEMART_CART_NOTIFY') . '<span></span></a>\');
				} else {
					...
					$("#productdetailsview .addtocart-bar2>div").remove();
					$("#productdetailsview .addtocart-bar2>a.notify").remove();
					$("#productdetailsview .addtocart-bar2").append(\'<div class="controls"><label for="quantity" class="quantity_box">'.JText::_('COM_VIRTUEMART_CART_QUANTITY').': </label><span class="box-quantity"><span class="quantity-box"><input type="text" class="quantity-input js-recalculate" name="quantity[]" value="\' + quantity + \'" /></span><span class="quantity-controls js-recalculate"><i class="quantity-controls quantity-plus">+</i><i class="quantity-controls quantity-minus">-</i></span></span></div><span class="addtocart_button2"><button type="submit" value="'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'" title="'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'" class="addtocart-button cart-click">'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'<span>&nbsp;</span></button></span><div class="clear"></div>\');
